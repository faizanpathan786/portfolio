 ...
 * (api/contact.ts) and the local Vite dev middleware (vite.config.ts).
...
/** Validate input and send the email via Resend. Returns an HTTP-shaped result. */
export async function sendContactEmail(input: ContactInput): Promise<SendResult> {
  const { name, email, message, botcheck } = input;

  // Honeypot — bots fill this; humans never see it.
  if (botcheck) return { status: 200, body: { success: true } };

  if (!name?.trim() || !email?.trim() || !message?.trim()) {
    return { status: 400, body: { error: "Please fill in all fields." } };
  }
  ...
  if (!apiKey) {
    ...
    return {
      status: 500,
      body: { error: "Email service is not configured yet." },
    };
  }
  ...
  try {
    ...
    return { status: 500, body: { error: "Unexpected error. Please try again." } };
  }
}
